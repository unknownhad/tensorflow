...
GPU attached. The label links to the AMD-owned runner fleet. The workflow
discovers the specific gfx target at runtime by calling `rocminfo`.

### Pinned Docker image

To make the runtime environment reproducible, the workflow has a tiny first job,
`rocm-config`, whose only purpose is to emit a pinned image digest as a job
output:
...
The human-readable tag is preserved in comments for clarity, but downstream jobs
use the sha256 digest. This prevents pipeline breakage if tags are mutated on
Docker Hub.

Updating the ROCm version requires changing the digest in `rocm-config`.
Downstream jobs consuming this output update automatically.

### Container options

Both downstream jobs share a container spec (anchored as `&rocm_container` in
YAML and reused via `*rocm_container`):
...
`timeout-minutes` is set at the job level, with a tighter `timeout-minutes:
60` (XLA single-GPU, JAX) or 80 (XLA CPU suite) on each test step.
...
2.  Download the AMD-maintained driver script (`execute_ci_build_upstream.sh`) from the `ROCm/xla` fork.
...
`rocm_xla.bazelrc` defines two pattern lists, `xla_sgpu` and `xla_mgpu`, used by
the legacy single- and multi-GPU helper scripts. They are not referenced
directly from `rocm_ci.yml` today (the workflow goes through the
`execute_ci_build_upstream.sh` script instead), but they document which
collective/sharding/iota tests are skipped in single-GPU mode and which ones
must be promoted to the multi-GPU runner:
...
… (and many other -//xla/tests:*_test patterns) …
```
...
Slots are filled across GPUs sequentially (e.g., Slot 0 on all GPUs first,
then Slot 1) to minimize contention before oversubscribing memory.
...
-   **`run_xla.sh`** — single-GPU XLA test sweep. Detects GPU count via
    ...
    `bazel test --config=rocm_ci --config=xla_sgpu …` with a similar tag
    filter recipe as CI (though they differ slightly in implementation
    details). Uses `--run_under=//build_tools/ci:parallel_gpu_execute` (the
    CUDA-style wrapper).
